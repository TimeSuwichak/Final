...
// ฟังก์ชันสร้าง PDF พร้อมลายเซ็นลูกค้า
export const generateSignedCompletionReportPdf = (
  job: Job,
  ...
  try {
    ...
    const signerNameLines = doc.splitTextToSize(`(${signerName})`, 60);
    doc.text(signerNameLines, 150, y + 5, { align: "center" });
    doc.text("ผู้ตรวจรับงาน / ลูกค้า", 150, y + 12, { align: "center" });
  } catch (e) {
  ...
  doc.save(`${job.id}-signed-completion-report.pdf`);
};
