...
import { admin as initialAdmins } from "@/data/admin"
...
const loadDataFromStorage = () => {
  ...
  return { jobs: initialJobs, users: initialUsers, leaders: initialLeaders, admins: initialAdmins }
}
...
export default function AdminDashboardPage() {
  ...
  const findAdminById = (id: number) => {
    return appData.admins?.find((a) => a.id === id)
  }
  ...
  const handleCreateJob = (event: React.FormEvent) => {
    ...
    const newJob = {
      ...
      createdByAdminId: currentUser?.id, // เพิ่มผู้สร้างงาน
      createdAt: new Date(), // เพิ่มเวลาที่สร้าง
    }
  ...
  const filteredJobs = useMemo(() => {
    // ถ้ายังไม่มี currentUser ให้ return array ว่างไปก่อน
    if (!currentUser) return []

    return appData.jobs
      .filter((job) => {
        // 1. กรองตาม "ผู้สร้างงาน" (งานของฉันเท่านั้น)
        if (job.createdByAdminId !== currentUser.id) return false

        // 2. กรองตาม "เดือน/ปี" ที่เลือก
        const jobDate = new Date(job.dates.start)
        if (jobDate.getMonth() !== filterDate.getMonth() || jobDate.getFullYear() !== filterDate.getFullYear()) {
          return false
        }

        // 3. กรองตาม "สถานะ" ของ Tab ที่เลือก
        if (activeTab === "new") {
          return job.status === "new" && !job.acknowledgedByLeader
        }
        if (activeTab === "in-progress") {
          return (job.status === "new" && job.acknowledgedByLeader) || job.status === "in-progress"
        }
        if (activeTab === "completed") {
          return job.status === "completed"
        }

        return true // กรณีอื่นๆ (ถ้ามี)
      })
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
  }, [appData.jobs, currentUser, activeTab, filterDate])

  // --- ✨ [แก้ไข] LOGIC การกรองงาน - แสดงเฉพาะงานที่ตัวเองสร้าง ✨ ---
  const myJobsWithAdminNames = useMemo(() => {
    if (!currentUser) return []

    return appData.jobs
      .filter((job) => {
        return job.createdByAdminId === currentUser.id
      })
      .map((job) => {
        // หา admin ที่สร้างงาน
        const creator = findAdminById(job.createdByAdminId)
        return {
          ...job,
          createdByAdminName: creator
  ...
  }, [appData.jobs, appData.admins, currentUser])
  ...
  return (
    <div className="flex-1 space-y-8 p-4 md:p-8">
      <div className="flex items-center justify-between">
        ...
        >
          <DialogTrigger asChild>
            ...
            <DialogHeader>
              <DialogTitle>{editingJob ? `แก้ไขใบงาน: ${editingJob.id}` : "สร้างใบงานใหม่"}</DialogTitle>
      ...
      {/* --- ✨ [ใหม่] เรียกใช้ Data Table โดยตรง ✨ --- */}
      <DataTable
        columns={columns}
        data={myJobsWithAdminNames} // ส่งข้อมูล "งานของฉัน" ที่กรองแล้วเข้าไป
        meta={{
          viewJob: (job) => setViewingJob(job),
          editJob: (job) => {
            setViewingJob(job)
            handleStartEdit()
          },
      ...
      <JobDetailsDialog
        ...
        creator={viewingJob ? findAdminById(viewingJob.createdByAdminId) : null}
        isOpen={!!viewingJob}
