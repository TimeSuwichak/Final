...
function DashboardFilters({ activeRange, onRangeChange }: any) {
  ...
  // [NOTE] ค่าสีพื้นหลังของ Filter Box ถูกปรับให้เป็น dark:bg-slate-800 เพื่อให้มีคอนทราสต์กับพื้นหลังหลัก #0D0E15
  const filterBoxStyle = "bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-lg px-4 py-2 min-w-[180px] transition-colors hover:border-gray-400 dark:hover:border-slate-600"
  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-lg p-1 flex space-x-1">
...
function KpiCardSkeleton() {
  return (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700">
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="h-4 bg-gray-200 dark:bg-slate-700 rounded w-3/4 mb-2"></div>
          <div className="h-10 bg-gray-300 dark:bg-slate-500 rounded w-1/2"></div>
        </div>
        <div className="p-3 rounded-full bg-gray-200 dark:bg-slate-700 w-12 h-12"></div>
      </div>
      <div className="h-3 bg-gray-200 dark:bg-slate-700 rounded w-full"></div>
    </div>
  )
}
function ChartSkeleton() { return (<div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700"><div className="h-6 bg-gray-300 dark:bg-slate-700 rounded w-1/2 mb-4"></div><div className="h-64 bg-gray-200 dark:bg-slate-700 rounded-lg"></div></div>) }
function TableSkeleton() { return (<div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700"><div className="h-6 bg-gray-300 dark:bg-slate-700 rounded w-1/3 mb-4"></div><div className="space-y-3"><div className="h-8 bg-gray-200 dark:bg-slate-700 rounded"></div><div className="h-8 bg-gray-200 dark:bg-slate-700 rounded"></div><div className="h-8 bg-gray-200 dark:bg-slate-700 rounded"></div><div className="h-8 bg-gray-200 dark:bg-slate-700 rounded"></div></div></div>) }
...
export default function ExDashboard() {
  ...
  const handleExportPDF = () => {
    ...
    // [แก้ไข] เปลี่ยนสีพื้นหลังในการ Export PDF ในโหมดมืดเป็น #0D0E15
    const options = { scale: 2, useCORS: true, backgroundColor: document.documentElement.classList.contains('dark') ? '#0D0E15' : '#f9fafb' }
  ...
  // [แก้ไข] เปลี่ยน dark:bg-slate-950 เป็น dark:bg-[#0D0E15]
