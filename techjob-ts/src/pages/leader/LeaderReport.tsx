...
const LeaderReport: React.FC = () => {
  ...
  // งานที่ leader จัดการ (leadId ตรงกับ user.id)
  const leaderJobs = jobs?.filter((job: any) => {
    if (!user) return false
    return String(job.leadId) === String(user.id)
  }) || []
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const relatedJob = relatedJobId ? leaderJobs.find((job: any) => job.id === relatedJobId) : null
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {leaderJobs.length > 0 ? (
                      ...
                      <div className="p-3 bg-muted rounded-lg border border-dashed">
                        <p className="text-sm text-muted-foreground">
                          คุณยังไม่มีงานที่รับผิดชอบ กรุณาติดต่อผู้ดูแลระบบ
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => navigate("/leader/leaderdashboard")}
