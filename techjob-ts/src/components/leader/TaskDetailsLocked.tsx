...
const StatusPill = ({
  ...
  return (
    <div
      className={`${config.bgColor} border-2 ${config.borderColor} rounded-lg p-4 text-center`}
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-sm font-medium text-gray-800 flex-1 line-clamp-1">
          {task.title}
        </span>
        <ChevronRight
          className={`h-4 w-4 ${
            config.textColor
          } shrink-0 transition-transform ${isExpanded ? "rotate-90" : ""}`}
        />
      </button>

      {isExpanded && (
        <div
          className={`${config.expandedBg} border-t ${config.borderColor} p-3 space-y-3`}
        >
          {task.description && (
            <div>
          ...
          {task.updates && task.updates.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-700 mb-2">
                ประวัติการแก้ไข ({task.updates.length})
              </p>
              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                {task.updates.map((update, idx) => (
                  <div
                    key={idx}
                    className="p-2 bg-white/70 rounded text-xs border border-gray-200"
                  >
...
export const TaskDetailsLocked: React.FC<TaskDetailsLockedProps> = ({
  tasks,
}) => {
      ...
      {inProgressTasks.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold text-orange-700 flex items-center gap-2">
            ...
            สถานะปัจจุบัน ({inProgressTasks.length})
          </h3>
          <div className=" grid space-y-1 gap-3">
            {inProgressTasks.map((task) => (
              <div
                key={task.id}
                className="flex-shrink-0 min-w-[200px] flex-1 max-w-[310px]"
              >
                <TaskCard
                  task={task}
          ...
          <div className="space-y-2">
            ...
            <div className=" flex flex-wrap gap-3">
              {completedTasks.map((task) => (
                <div
                  key={task.id}
                  className="flex-shrink-0 min-w-[200px] flex-1 max-w-[300px]"
                >
                  <TaskCard
                    task={task}
                    isExpanded={expandedTaskId === task.id}
