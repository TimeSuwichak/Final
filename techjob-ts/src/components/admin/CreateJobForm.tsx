...
export function CreateJobForm({ users, leaders, onSubmit }: { users: any[], leaders: any[], onSubmit: (data: any) => void }) {
    const allDepartments = [...new Set(leaders.map(l => l.department))];
    ...
    const availableLeads = useMemo(() => {
        ...
        // กรอง 2 ชั้น: 1. อยู่แผนกเดียวกัน และ 2. สถานะต้องเป็น 'available'
        return leaders.filter(l =>
            l.department === selectedDepartment && l.status === 'available'
        );
    }, [selectedDepartment, leaders]); // <-- เพิ่ม leaders ใน dependency array

    const availableTechs = useMemo(() => {
        if (!selectedDepartment) return [];
        // กรอง 2 ชั้นเหมือนกัน
        return users.filter(u =>
            u.department === selectedDepartment && u.status === 'available'
        );
    }, [selectedDepartment, users]); // <-- เพิ่ม users ใน dependency array
