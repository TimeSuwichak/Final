...
export function MaterialSelectionDialog({
  ...
  const handleQuantityChange = (materialId: string, quantity: number) => {
    ...
    if (inventoryMaterial && quantity > inventoryMaterial.stock) {
      alert(`สต็อก "${inventoryMaterial.name}" คงเหลือ ${inventoryMaterial.stock} ${inventoryMaterial.unit}`);
                    ...
                    {currentCategoryData.map((material) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => !isOutOfStock && handleMaterialToggle(material)}
                        >
                          <div className="flex-1">
                            <p className="font-medium text-sm">{material.name}</p>
                            <p className="text-xs text-muted-foreground">
                              รหัส: {material.id} • หน่วย: {material.unit || 'ชิ้น'} • คงเหลือ {material.stock}
                            </p>
                          </div>
                          {isSelected && (
