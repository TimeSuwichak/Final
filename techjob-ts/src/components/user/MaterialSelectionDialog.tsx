...
export function MaterialSelectionDialog({
  ...
  const materialCategories = [
    { id: 'electrical', name: 'วัสดุไฟฟ้า', data: electricalMaterials },
    { id: 'network', name: 'วัสดุเครือข่าย', data: networkMaterials },
    { id: 'tools', name: 'เครื่องมือ', data: toolMaterials },
    { id: 'multimedia', name: 'วัสดุมัลติมีเดีย', data: multimediaMaterials },
    { id: 'consumables', name: 'วัสดุสิ้นเปลือง', data: consumableMaterials },
  ];

  const currentCategoryData = materialCategories.find(cat => cat.id === selectedCategory)?.data || [];
  ...
  const handleMaterialToggle = (material: any) => {
    const materialId = `${selectedCategory}-${material.id}`;
  ...
  const handleWithdraw = () => {
    ...
    updateJobWithActivity(
      ...
      { tasks: updatedTasks },
      'other',
      `เบิกวัสดุสำหรับ Task: ${task.title} - ${selectedMaterials.map(m => `${m.name} (${m.quantity})`).join(', ')}`,
      user.fname,
      'tech',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {selectedCategory && (
              <div className="space-y-2">
                ...
                <ScrollArea className="h-96 border rounded-lg p-2">
                  <div className="space-y-2">
                    {currentCategoryData.map((material: any) => {
                      const materialId = `${selectedCategory}-${material.id}`;
                      const isSelected = selectedMaterials.some(m => m.id === materialId);
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              รหัส: {material.id} • หน่วย: {material.unit || 'ชิ้น'}
                            </p>
                          </div>
                          {isSelected && (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">วัสดุที่เลือก ({selectedMaterials.length})</Label>
              ...
              {selectedMaterials.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedMaterials.map((material) => (
                    <div key={material.id} className="flex items-center justify-between p-3 border rounded-lg bg-green-50 border-green-200">
                      <div className="flex-1">
                        <p className="font-medium text-sm">{material.name}</p>
                        <p className="text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleMaterialToggle({ id: material.id.split('-')[1] });
                          }}
                        >
