...
export default function UnifiedChatInterface({ currentUserId }: UnifiedChatInterfaceProps) {
    ...
    const allUsers = useMemo(
        () => [
            ...userData.map((u) => ({ ...u, id: String(u.id), role: u.role || "user" })),
            ...leaderData.map((u) => ({ ...u, id: String(u.id), role: u.role || "leader" })),
            ...adminData.map((u) => ({ ...u, id: String(u.id), role: u.role || "admin" })),
            ...executiveData.map((u) => ({ ...u, id: String(u.id), role: u.role || "executive" })),
        ],
        []
    );
    ...
    useEffect(() => {
        ...
        // 1. โหลดจาก localStorage ก่อนเพื่อแสดงทันที
        const cacheKey = `chat_list_${currentUserId}`;
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            try {
                const cachedChats = JSON.parse(cached);
                console.log('[UnifiedChatInterface] Loaded from cache:', cachedChats.length, 'chats');
                setChats(cachedChats);
            } catch (e) {
                console.error('[UnifiedChatInterface] Failed to parse cached chats:', e);
            }
        }
        ...
        const unsub = onSnapshot(q, (snap) => {
            ...
            console.log('[UnifiedChatInterface] Loaded from Firestore:', chatList.length, 'chats');
            setChats(chatList);

            // 3. บันทึกลง localStorage
            try {
                localStorage.setItem(cacheKey, JSON.stringify(chatList));
            } catch (e) {
                console.error('[UnifiedChatInterface] Failed to save chats to cache:', e);
            }
        });
    ...
    const filteredUsers = useMemo(() => {
        console.log('[UnifiedChatInterface] Filtering users. Current user ID:', currentUserId);
        ...
        const filtered = allUsers.filter((u) => {
            ...
            if (userId === currentId) {
                console.log('[UnifiedChatInterface] Filtering out current user:', u.fname, u.lname, 'ID:', userId);
                return false;
            }
        ...
        console.log('[UnifiedChatInterface] Filtered users count:', filtered.length);
        return filtered;
    ...
    const filteredChats = useMemo(() => {
        return chats
            .map((chat) => {
                ...
                const displayName = otherUser ? `${otherUser.fname} ${otherUser.lname}` : otherUserId;
            ...
            .filter((chat) => {
                ...
                if (searchText.trim() && !chat.displayName.toLowerCase().includes(searchText.toLowerCase())) {
                    return false;
                }
                return true;
            });
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                <button
                                    ...
                                    className={`px-3 py-1 rounded text-sm transition ${selectedRole === null
                                ...
                                {availableRoles.map((role) => (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded text-sm transition ${selectedRole === role
                                            ? "bg-blue-600 text-white"
                                            : "bg-muted text-foreground hover:bg-muted/80"
                                            }`}
                                    >
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2 mt-2">
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded text-sm transition ${selectedDepartment === null
                                    ...
                                    {availableDepartments.map((dept) => (
                                        <button
                                            ...
                                            className={`px-3 py-1 rounded text-sm transition ${selectedDepartment === dept
                                                ? "bg-green-600 text-white"
                                                : "bg-muted text-foreground hover:bg-muted/80"
                                                }`}
                                        >
                                            {dept && departmentLabels[dept as string] ? departmentLabels[dept as string] : dept}
                                        </button>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="text-xs text-muted-foreground">{u.position || roleLabels[u.role] || u.role}</div>
                                                {u.department && (
                                                    <div className="text-xs text-muted-foreground">
                                                        {departmentLabels[u.department] ? departmentLabels[u.department] : u.department}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {chat.otherUser && (
                                                    <p className="text-xs text-muted-foreground">
                                                        {chat.otherUser.position || (chat.otherUser.role && roleLabels[chat.otherUser.role]) || chat.otherUser.role}
                                                    </p>
                                                )}
                                                {chat.lastMessage && (
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-slate-900 dark:text-white">
                                        {selectedUser ? `${selectedUser.fname} ${selectedUser.lname}` : selectedUserId}
                                    </h3>
                                    {selectedUser && (
                                        <p className="text-sm text-muted-foreground">
                                            {selectedUser.position || (selectedUser.role && roleLabels[selectedUser.role]) || selectedUser.role}
                                        </p>
                                    )}
                                </div>
                            </div>
